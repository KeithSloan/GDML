    ...
    for grp in mat_obj.Group:
        ...
        if sym is not None:  # The mixture involves a natural element
            ...
            for component in component_list:
                ...
                component[
                    'fraction'] *= weight_fraction * isotopeObject.atomic_weight / isotopeObject.element.atomic_weight
                component['type'] = 'wo'
        ...
        if elem_grp is not None:
            for isotope in elem_grp.Group:
                ...
                nuc_dict[
                    'fraction'] = weight_fraction * isotope_fraction * isotopeObject.atomic_weight / isotopeObject.element.atomic_weight
                nuc_dict['type'] = 'wo'
                nuclide_list.append(nuc_dict)
            continue
        ...
        if mat_grp is not None:
            ...
            for component in component_list:
                ...
                if component['type'] == 'ao':
                    ...
                    component[
                        'fraction'] *= weight_fraction * nuclideObject.atomic_weight / nuclideObject.element.atomic_weight
...
def exportGDML(first, filepath, fileExt):
    ...
    if fileExt == ".xml":
        ...
        if platform == "win32":
            ...
            ET.ElementTree(materials).write(materials_file_path, pretty_print=True, xml_declaration=True,
                                            encoding='UTF-8')
    ...
    if fileExt.lower() == ".xml":
        ...
        #    exportMaterials(first, filepath)
            ...
            QtGui.QMessageBox.critical(
                None, "Need to select a Part for export",
                "Need to select Part of GDML Volume to be exported \n\n Press OK to return"
            )
        # profiler.disable()
...
class EllipticalConeSurfaceExporter(SurfaceExporter):
    ...
    def copy(self):
        return EllipticalConeSurfaceExporter(self.name, self.saved_center, self.saved_dx, self.saved_dy,
                                             self.saved_zHeight)
...
class EllipsoidSurfaceExporter(SurfaceExporter):
    ...
    def copy(self):
        return EllipsoidSurfaceExporter(self.name, self.saved_ax, self.saved_by, self.saved_cz, self.saved_center,
                                        self.saved_rotation)
...
class ParaboloidSurfaceExporter(SurfaceExporter):
    ...
    def copy(self):
        return ParaboloidSurfaceExporter(self.name, self.saved_rlo, self.saved_rhi, self.saved_dz, self.saved_center,
                                         self.saved_rotation)
...
class BoxExporter(SolidExporter):
    ...
    def generate_surfaces(self):
        ...
        self.region = Region(
            f"+{x1surface.id} -{x2surface.id} +{y1surface.id} -{y2surface.id} +{z1surface.id} -{z2surface.id}")
    ...
    def generate_surfaces(self):
        ...
        if angle != 360:
            ...
            if phi0_normal.cross(phi1_normal).z > 0:
                self.region = Region(
                    f'-{surface.id} +{bottom_plane.id} -{top_plane.id} +{phi0_plane.id} -{phi1_plane.id}')
            else:
                self.region = Region(
                    f'-{surface.id} +{bottom_plane.id} -{top_plane.id} (+{phi0_plane.id} | -{phi1_plane.id})')
...
class GDMLTorusExporter(SolidExporter):
    def __init__(self, obj):
        ...
        N = int(
            2 * math.pi / (2 * thet))  # number of divisions of the circle that that given a maximum radial difference
...
class TorusExporter(SolidExporter):
    ...
    def get_verts(self):
        ...
        verts += [center + radius * Vector(math.cos(angle1 + i * dthet), 0, -math.sin(angle1 + i * dthet)) for i in
                  range(N)]
...
class OrthoArrayExporter(SolidExporter):
    ...
    def generate_surfaces(self):
        ...
        if rotatePosition:
            print(
                f" Will apply position rotation to {base.Label} because global rotation is different than base rotation")
        ...
        for i, face in enumerate(self.mesh.Facets):
            ...
            for j in range(n):
                ...
                if edge_len < ShapeMesher.deflection:
                    print(
                        f"Warning  edge {j} of face {i} is shorter than deflection: edge length = {edge_len}  deflection = {ShapeMesher.deflection}")
                    continue
